...
# create schema with hybrid search support
...
sparse_vector = FieldSchema(name="sparse_vector", dtype=DataType.SPARSE_FLOAT_VECTOR)
...
schema = CollectionSchema([
    id, content, vector, user_id, document_id, sparse_vector, document_name_field, page_number_field
])
...
# Dense vector index (using IP for hybrid compatibility)
vector_index = {"index_type": "FLAT", "metric_type": "IP"}

# Sparse vector index (using IP for hybrid compatibility)
...
public_collection.create_index("sparse_vector", sparse_index)
...
private_collection.create_index("sparse_vector", sparse_index)
...
if not public_collection.has_index(index_name="vector") or not private_collection.has_index(index_name="vector"):
    raise Exception("Public or private vector indexes are not loaded")
if not public_collection.has_index(index_name="sparse_vector") or not private_collection.has_index(index_name="sparse_vector"):
    raise Exception("Public or private sparse vector indexes are not loaded")
print(utility.list_collections())
print(f"Database with hybrid search capabilities has been created successfully")